...
export const AvoidanceSimulator: React.FC<AvoidanceSimulatorProps> = ({
  ...
  const [simTimeSeconds, setSimTimeSeconds] = useState<number>(2535);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="p-6 rounded-2xl glass-panel border border-cyan-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm font-telemetry text-slate-300 mt-1 leading-relaxed">
            Simulate thruster ΔV impulses & evaluate trajectory clearance for SAT-01.
          </p>
        </div>

        <div className="flex items-center gap-2 bg-amber-950/40 px-3.5 py-2 rounded-xl border border-amber-500/30 text-amber-300 text-xs font-telemetry font-bold shrink-0">
          <Info className="w-4 h-4 text-amber-400 shrink-0" />
          <span>PHYSICAL SGP4 ORBITAL SIMULATION ENGINE (PROTOTYPE)</span>
        </div>
      </div>

      {/* Main Grid: xl:grid-cols-12 ensures maneuver options and canvas stack comfortably on laptops/tablets */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          {MOCK_MANEUVER_OPTIONS.map((opt) => {
            ...
            return (
              <div
                ...
                className={`p-5 rounded-2xl cursor-pointer transition-all border min-h-fit ${
                  isSelected
                    ? opt.id === 'OPTION_A'
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <span className={`text-[10px] font-telemetry font-bold px-2 py-0.5 rounded border uppercase inline-block ${
                      opt.id === 'OPTION_A'
                    ...
                    <h3 className="font-heading font-extrabold text-base text-white mt-1.5 leading-snug">
                      {opt.subTitle}
                    </h3>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 pt-3 border-t border-slate-800/80 text-xs font-telemetry">
                  <div>
                    <span className="text-slate-400 block text-[10px] font-bold">ΔV REQUIRED</span>
                    <span className="font-bold text-white text-sm mt-0.5 block">{opt.deltaV}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block text-[10px] font-bold">PREDICTED MISS</span>
                    <span className="font-bold text-cyan-400 text-sm mt-0.5 block">{opt.predictedMissDistanceKm} km</span>
                  </div>
                </div>
              </div>
            );
        ...
        {/* Right Canvas & Telemetry Comparison Column */}
        <div className="xl:col-span-7 w-full space-y-4">
          ...
          <div className="relative w-full h-80 rounded-2xl glass-panel border border-cyan-500/30 overflow-hidden">
            <div className="absolute top-3 left-3 right-3 z-10 flex items-center justify-between font-telemetry text-xs gap-2">
              <div className="text-cyan-400 bg-space-950/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-cyan-500/30 flex items-center gap-2">
                ...
                <span className="text-[10px] text-slate-400 hidden sm:inline">(14:32:15 UTC)</span>
              </div>
          ...
          <div className="p-5 rounded-2xl glass-panel border border-cyan-500/25 space-y-4 font-telemetry">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
              <div className="p-3.5 rounded-xl bg-space-900/90 border border-slate-800 space-y-1">
                <span className="text-slate-400 block text-[10px] font-bold">PARAMETER</span>
                <span className="font-bold text-white block">Miss Distance</span>
                <span className="font-bold text-white block">Collision Risk</span>
                <span className="font-bold text-white block">Thruster Fuel</span>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-red-950/20 border border-red-500/30 space-y-1">
                ...
                <span className="text-slate-300 block">14.2 km</span>
              ...
              <div className="p-3.5 rounded-xl bg-emerald-950/20 border border-emerald-500/30 space-y-1">
                ...
                <span className="text-cyan-300 font-bold block">+{selectedOption.predictedMissDistanceKm} km</span>
